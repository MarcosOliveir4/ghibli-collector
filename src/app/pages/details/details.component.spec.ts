...
const sut = async () => {
  return await render(DetailsComponent, {
    imports: [RouterLink, RouterLinkActive, NgOptimizedImage, PipeModule],
    providers: [
...
describe('DetailsComponent', () => {
  ...
  it('should call getFilmById on ngOnInit', async () => {
    const { fixture } = await sut();
    fixture.componentInstance.ngOnInit();
    expect(ghibliApiServiceMock.getFilmById).toHaveBeenCalled();
  });

  it('should fetch film on getFilmById', async () => {
    const { fixture } = await sut();
    fixture.componentInstance.getFilm();
    expect(fixture.componentInstance.film).toEqual(mockFilm);
    expect(fixture.componentInstance.loadingFilm).toBe(false);
  });

  it('should cancel subscription on ngOnDestroy', async () => {
    const { fixture } = await sut();
    fixture.componentInstance.ngOnDestroy();
    expect(fixture.componentInstance.subscription.closed).toBe(true);
  });
  it('should go back on goBack', async () => {
    await sut();
    const btnBackToHome = screen.getByTestId('btn-back-to-home');
    await userEvent.click(btnBackToHome);
